...
*Retrieval practice, not assessment. Answer aloud or in writing from memory before expanding — the effort of recalling is what does the work. Nothing is recorded anywhere.*
...
<details><summary>Name the four numbers this course asks you to log per task.</summary>

Wall-clock to merged (not time the agent ran); rework passes; read ratio (fraction of the shipped diff you actually read); verification events (times something other than you or the model confirmed correctness).

</details>

<details><summary>Why must the prediction be written before the task rather than after?</summary>

A prediction written afterward is not a prediction. It silently destroys every calibration measurement downstream, and calibration is the thing M1 is training.
...
<details><summary>What are the three intervention types in the session taxonomy, and which one rising is the warning sign?</summary>

Steering (redirecting a fine approach), correcting (fixing a wrong output), rescuing (aborting). A rising *rescue* share means your briefs or your context have degraded.
...
*Goal of this phase: drive one agent, well, on real work, with a loop that catches your mistakes.*
...
- "It went in circles" means: the stop condition never fired and nothing in context changed between iterations. **Loops that cannot change their own context cannot terminate usefully** — this is the seed of M11.
...
**L2.2 — Break it deliberately (⏱ 1–2 h).** Four experiments, each ~15 minutes, each with a written prediction *before* you run it:
...
**L2.3 — Add one harness feature (⏱ 2 h).** Pick one and implement it: (i) a permission prompt before `bash`, (ii) naive compaction — when messages exceed N tokens, summarize the oldest half, (iii) a `spawn_subagent` tool that runs a fresh loop and returns only its final message.
*Success:* it works, plus 200 words on what it cost you (latency, tokens, correctness risk).

**L2.4 — Harness teardown (⏱ 1 h).** Take the agent harness you use daily. Map its features onto your loop: where does its planning mode intervene? What does compaction actually remove? What does a subagent return to the parent, and what is lost?
...
### Common failure modes
Copy-pasting the tutorial without typing it. Adding features before the bare loop works. Skipping L2.2 — it is where the learning is.

### Self-check

*Retrieval practice, not assessment. Answer aloud or in writing from memory before expanding — the effort of recalling is what does the work. Nothing is recorded anywhere.*

<details><summary>Complete the sentence: an agent is an LLM, a loop, and ___.</summary>

Enough tokens. Thorsten Ball's formulation — demonstrable in under 400 lines.

</details>

<details><summary>The agent claims a file exists that does not. Explain the failure in loop terms, without using the word 'hallucinate'.</summary>

No tool result in the message list contradicted it. The model never sees your filesystem — it sees tool results you appended to a list.

</details>

<details><summary>Why can a loop that cannot change its own context never terminate usefully?</summary>

The stop condition is evaluated against context. If nothing in context changes between iterations, the same input produces the same decision forever. This is the seed of the loop-engineering module.

</details>

<details><summary>Two harnesses report scores 15 points apart on the same benchmark with the same base model. What does that tell you?</summary>

That the harness co-authors the number. Scaffolding moves benchmark results by double digits, which is the first reason public benchmarks cannot predict your outcomes.

</details>

<details><summary>Name two ways a badly designed tool *result* breaks an agent.</summary>

An 8,000-token build log is a context bomb (it evicts everything useful); silent truncation is a correctness bomb (the agent reasons confidently from a partial picture with no marker telling it so).
...
| 10+ files, hours | full brief + research phase | always (M7) | worktree/container | review plan, then check in |
...
**Context poisoning and the case for restarting.** Once a wrong approach is in context, the model will keep referencing it — it is *evidence* in the message list (M2). Corrections pile new instructions on top of visible failed reasoning. Harnesses that let you rewind to a prior state and re-brief are removing the failed attempt from context rather than arguing with it. Cherny's formulation: rewind instead of correcting, to keep failed attempts out of context. The general rule:
...
**L3.1 — Brief archaeology (⏱ 2 h).** Take five of your recent agent sessions that went badly. For each, write the brief you *should* have written, using the four parts. Then re-run two of them with the improved brief in a clean session. Log both (M1 instrument).
*Success:* five rewritten briefs; ≥1 measurable improvement in rework passes; a note on the one that didn't improve and why.

**L3.2 — Blast-radius calibration (⏱ 2 h).** For ten upcoming tasks, predict file count and duration before starting; record actuals. Build your personal correction factor.
...
**L3.3 — The abandonment drill (⏱ 1.5 h).** Run five tasks with a hard rule: at the *second* correction on the same issue, you must discard and restart with a rewritten brief. No exceptions, even when it feels close.
...
**L3.4 — Modality experiment (⏱ 1.5 h).** Do three visual/UI tasks with text-only descriptions, then three comparable ones with screenshots. Separately, dictate three briefs by voice and type three. Log all twelve.
...
<details><summary>What are the four parts of a brief, and which one do people omit?</summary>

Goal (in outcomes, not steps), constraints, acceptance criteria, and verification means. The fourth is the one people omit and the one that matters most.

</details>

<details><summary>State the abandonment rule.</summary>

First correction: steer. Second correction on the same issue: stop, discard the context, rewrite the brief. Steering a bad trajectory costs more than restarting.

</details>

<details><summary>Why is correcting in-context often worse than starting clean?</summary>

The failed approach is *evidence* in the message list. Corrections pile new instructions on top of visible failed reasoning, and the model keeps referencing it.

</details>

<details><summary>The agent gets one of your conventions wrong. Where does the fix belong, and where does it not?</summary>

In the memory file or a skill, once. Not in the chat — a chat correction fixes this session and nothing else.

</details>

<details><summary>What single estimate, made before you start, determines brief length, whether to plan, whether to isolate, and whether you can walk away?</summary>

Blast radius — how many files the change touches and roughly how long it will take.
...
⊢ M2 · ⏱ 8–12 h · **This is the keystone module of Phase 2.**
...
- Run mutation testing occasionally (M14) — the honest answer to "are these tests real?"
- A hook that fails the session if test files were modified during an implementation-only task (M9) turns a request into an enforcement.
...
**Verification is not review.** A verifier answers "does it do what it claims?" Review answers "is this the right thing, and will we regret it?" M5 is the second question. Do not let a green suite substitute for it.

### Labs

**L4.1 — The verifier audit (⏱ 2 h).** Take your last 15 agent-assisted changes. Score each on the ladder. Compute the distribution.
*Success:* a table plus a written diagnosis of your weakest rung and the cheapest way to raise it.

**L4.2 — Build the fast lane (⏱ 3–4 h).** In your main repo, create a target that runs the most informative possible subset of checks in **under 15 seconds** — type check, lint, a fast unit subset. Wire it into your agent's memory file as the command to run after every edit. Add a post-edit hook if your harness supports one.
*Success:* `<15s` measured, documented, and the agent actually runs it unprompted in a fresh session.

**L4.3 — Same-context tests (⏱ 2 h).** Implement three features. For each, ask for tests in the same session immediately after. Then, for a fourth, implement in one session and ask for tests in a *fresh* session. Compare: assertion quality, tautologies, bugs found.
*Success:* four test suites with a written comparison, including at least one identified tautological test.

**L4.4 — Adversarial verification (⏱ 2 h).** Give an agent a task and a deliberately weak test. Instruct it only to "make the tests pass." Document every way it games the verifier. Then strengthen the verifier and repeat.
*Success:* a catalogue of ≥3 gaming behaviours you personally observed, and the guardrail for each.

**L4.5 — Close a non-test loop (⏱ 2 h).** Pick a task where tests are a bad fit — a UI change, a CLI's output format, a report's appearance. Give the agent a way to *observe its own output*. Iterate until it self-corrects at least once without you.
...
<details><summary>Why is an agent with a working verifier categorically different from one without, rather than just better?</summary>

With a verifier it is doing search with a fitness function — an optimizer. Without one it is generating plausible text and stopping when it feels done — a random walk.

</details>

<details><summary>Name four rungs of the verifier ladder in ascending order.</summary>

Any ascending four from: nothing → compiler/types → linter → existing tests → new tests for this change → property/fuzz tests → running the software → preview deploy → domain-expert human review.

</details>

<details><summary>Why ask for tests in the same session as the implementation?</summary>

The model still holds the implementation reasoning in context, so the tests are better and bugs surface during writing. Tests written fresh against finished code tend to be tautological restatements of what the code does.

</details>

<details><summary>List three ways an agent told to 'make the tests pass' can comply without doing what you meant.</summary>

Weaken the assertion; delete or skip the test; special-case the test input; mock the thing under test. Kent Beck watches specifically for disabled tests and unrequested functionality.

</details>

<details><summary>Steinberger says he barely reads code. What makes that survivable for him, and why is copying the behaviour alone dangerous?</summary>

Verification stacked elsewhere: agent-written tests in the same context, continuous builds, two-minute preview deploys, atomic revertible commits, and playing with the running software. Not reading is a position you earn by building verification, not a shortcut taken instead of it.

</details>

<details><summary>Why does a 5-second test run 40 times beat a 20-minute suite run twice?</summary>

Iteration frequency is what turns the loop into an optimizer. Optimising a suite for agent iteration frequency is a different objective from the one most suites were built for.
...
6. **New dependencies** — supply chain (M13); agents are documented to under-verify packages
...
- Track read ratio (M1) as a first-class metric
...
**L5.1 — Risk-ordered review drill (⏱ 2 h).** Take five recent agent-generated PRs. Review each twice: once linearly for 10 minutes, once risk-ordered for 10 minutes. Record findings per method.
...
**L5.2 — Comprehension audit (⏱ 2 h).** List every module you shipped in the last 90 days. Rate your comprehension 0–3 without opening the file. Then open three low scorers and check whether you were right. Write a repayment plan for the worst one and execute it.
*Success:* audit table, calibration check, one repaid module.

**L5.3 — Dual review (⏱ 2 h).** Build an agent reviewer with an explicit checklist (your risk order, your conventions, your past incidents). Run it on ten PRs alongside your own review. Track: agent-only findings, human-only findings, both, and false positives.
*Success:* a confusion table and a revised checklist based on what it missed.

**L5.4 — The upstream experiment (⏱ 2 h).** For one substantial change, spend your entire review budget on the plan (M7 previews this) and none on the diff. For a comparable change, spend it all on the diff. Compare outcomes and your own anxiety level.
...
<details><summary>Give the risk-ordered review sequence, first three categories.</summary>

Deletions and test changes; security and boundary surface (auth, input handling, permissions, secrets, untrusted input); data and migrations. Then concurrency/money/time zones, public interfaces, new dependencies, everything else.

</details>

<details><summary>If tests catch bugs better than review does, what is review actually for?</summary>

Maintaining mental alignment about what changed and why. That reframing is what makes upstream review of research and plans coherent.

</details>

<details><summary>What is comprehension debt, and why is it more dangerous than technical debt?</summary>

The gap between code shipped and code understood. It is invisible in the codebase and lives only in your head, so you discover it during an incident rather than during planning.

</details>

<details><summary>Why should a different context review the code than wrote it?</summary>

The same context is grading its own homework — it will defend its earlier reasoning. A fresh or differently-instructed context is doing real work.

</details>

<details><summary>DHH reads beauty as a correctness signal. Where does that hold and where does it break?</summary>

Evidence for local correctness; weak evidence for architectural correctness. Agent output is often superficially beautiful and conceptually wrong, because surface style is exactly what a language model is best at.

</details>

<details><summary>Name the five conditions that must all hold before not reading a diff is defensible.</summary>

Reversible in seconds; contained blast radius; a strong verifier ran; not security-, money- or data-critical; and you find out fast if it is wrong.

</details>

### Reading
[R-40] Horthy, ACE-FCA (review-the-plan section) · [R-101] Osmani on comprehension debt · [R-03] DORA 2026 (verification tax, instability) · [R-45] DHH on risk-tiered review (current) · [R-43] DHH's April 2026 workflow (superseded in part)

---

### ▸ GATE 1 — Operator

Before Phase 3, demonstrate on a single recorded session in a real repo: a blast-radius assessment, a four-part brief, an agent-runnable verifier, one abandonment-and-restart, a risk-ordered review, and a logged entry in `ase/log.md`. See `ASSESSMENT.md` §Gate 1. **Capstone A** is taken here.
...
*Goal of this phase: stop compensating for a hostile environment. Shape the context, the process, and the codebase so the agent is set up to succeed before it starts.*
...
**L6.1 — Context census (⏱ 1.5 h).** In a real session, inspect what is actually in your window at startup: memory files, skill descriptions, tool definitions, system prompt. Produce a token breakdown.
*Success:* a pie chart or table of startup context with a total, plus one item you removed as a result.

**L6.2 — The always-on audit (⏱ 2 h).** Take your memory file section by section. For each, find evidence in your last ten sessions that it mattered. Delete or relocate anything without evidence. Re-measure.
*Success:* before/after line counts and token counts, with a relocation map (what became a skill, a rule, or nothing).

**L6.3 — Intentional compaction (⏱ 2 h).** Take a task that will exceed one window. Rather than letting auto-compaction fire, stop at ~50% utilization, write the structured compaction artifact to a file, start fresh from it, and finish. Then repeat a comparable task letting auto-compaction handle it.
*Success:* both transcripts, the artifact, and a comparison of what each approach lost.

**L6.4 — Rot diagnosis (⏱ 1.5 h).** Deliberately run a session to near-exhaustion on a real task. Log the *first* moment each rot symptom appears with its utilization percentage. Build your personal early-warning threshold.
*Success:* a symptom timeline and a stated threshold you will act on.

**L6.5 — Firewall a subagent (⏱ 2 h).** Take a high-volume research task. Run it in the main context, measure tokens consumed. Then run it via an isolated subagent with a specified return format. Compare cost and answer quality.
...
<details><summary>Give the optimization priority for a context window, in order.</summary>

Correctness (no wrong information) > completeness (no missing information) > size (no noise). Size is last — a lean context full of stale information is worse than a fat accurate one.

</details>

<details><summary>List the rot symptoms in the order they typically appear.</summary>

Stops following conventions → re-reads files it already read → re-proposes rejected approaches → contradicts earlier decisions → becomes confidently wrong about the codebase.

</details>

<details><summary>What goes into a compaction artifact?</summary>

End goal; approach being taken; steps completed; current blocker or failure state; critical file paths and dependencies. Written to a file, not just into the chat.

</details>

<details><summary>Name two things you must never compact away.</summary>

Decisions with their rationale, and rejected approaches *with reasons* — otherwise they get re-proposed. Also exact file paths, acceptance criteria, and the failure state verbatim.

</details>

<details><summary>Why is the main reason to spawn a subagent not parallelism?</summary>

Context isolation. A task may burn 50k tokens of searching to produce a 500-token answer, and you want only the answer. Design the return to look like a compaction artifact.

</details>

<details><summary>Steinberger runs an ~800-line agent file; current guidance says keep it under ~200. Both are right. Why?</summary>

The principle is not a line count — always-on content must earn its place on *every single request*. In his single enormous product codebase nearly all of it applies to nearly every task. The test is: did this matter in the last ten sessions?
...
**Where the ratio should land.** The reported 3:4 research-plan to implementation ratio is a reasonable starting point for substantial work in unfamiliar code. It is absurd for a two-line fix. Calibrate by blast radius (M3). The failure mode on each side is real: too little planning yields confident wrong direction; too much yields plans nobody reads, which is ceremony that feels like rigour.
...
**L7.1 — Research a stranger's codebase (⏱ 2 h).** Pick a real open-source project you have never worked in, and a real open issue. Run a research-only pass with no edit permission. Produce a research document. Verify three of its claims yourself against the source.
*Success:* research doc; ≥2 of 3 spot-checks accurate; any inaccuracy diagnosed (what in the process produced it?).

**L7.2 — Plan review drill (⏱ 2 h).** From that research, generate a plan. Review it and write down every objection *before* implementation. Implement. Then check which objections were real.
*Success:* objection list with hit rate, and a note on which class of objection you are good and bad at predicting.

**L7.3 — Ratio experiment (⏱ 3 h).** Take two comparable medium tasks. On one, spend ~40% of effort on research and planning. On the other, brief and go. Compare: wall-clock to done, rework passes, read ratio, your confidence.
*Success:* logged comparison with an honest verdict, including the case where planning lost.

**L7.4 — Plan reuse (⏱ 1.5 h).** Take a plan from a completed change and use it as the template for a similar change. Measure the savings and note what had to be re-researched.
*Success:* a derived plan, a time comparison, and a statement of what generalizes.

**L7.5 — Spec-driven trial (⏱ 2 h, optional).** Run one feature through a spec-driven toolkit end to end. Write an honest assessment: what did the ceremony buy, and what did it cost?
...
<details><summary>State the leverage argument for reviewing plans instead of diffs.</summary>

A bad line of code is a bad line of code. A bad line of a plan becomes hundreds of bad lines of code. A bad line of research becomes thousands.

</details>

<details><summary>What is the review question for the research phase, and what do you do if the answer is no?</summary>

Is this an accurate map of reality? Spot-check three claims against the actual source. If research is wrong, stop — everything downstream is poisoned.

</details>

<details><summary>What is the review question for the plan?</summary>

If a competent stranger executed this literally, would the result be right? This is where scarce attention buys the most.

</details>

<details><summary>Why do research and plan artifacts belong in files rather than in the conversation?</summary>

Durability across crashes and sessions; reviewability by humans and other agents; reuse on the next similar change; and they are the honest record of intent when the incident review happens months later.

</details>

<details><summary>The method failed on a Hadoop dependency removal. What does that failure teach?</summary>

It amplifies insufficient research rather than rescuing it. Three hours of planning on a bad map produces a confident wrong direction faster.

</details>

<details><summary>When is this workflow the wrong tool?</summary>

Small changes (planning a two-line fix is ceremony) and genuinely exploratory work, where you don't yet know what you want and the point is to find out by building.
...
4. Is there a <15s feedback command? (M4)
...
**Docs as context, not decoration.** Structure documentation so an agent reads *the right subsystem* — an index that maps concerns to directories, per-subsystem notes near the code, plan and research artifacts from M7 kept in the repo. Docs in a wiki the agent cannot reach are docs that do not exist.
...
**The measurement.** Legibility work should show up in your M1 numbers — fewer rework passes, fewer "the agent couldn't find it" sessions, higher first-pass success. If it doesn't, you improved your aesthetics, not your legibility.

### Labs

**L8.1 — Legibility audit (⏱ 2 h).** Score your primary repo on all ten checks with evidence for each score. Rank fixes by (impact × frequency) ÷ cost.
*Success:* scored audit with a ranked, costed remediation list.

**L8.2 — Automated hygiene sweep (⏱ 3 h).** Run dead-code and duplicate-code detection. Have agents remove dead code and consolidate the worst duplication, in small verified commits.
*Success:* merged cleanup with all tests green and a before/after metric.

**L8.3 — Kill the flakes (⏱ 3 h).** Identify flaky tests by running the suite ten times. Fix or quarantine every one.
*Success:* ten consecutive identical runs, plus a quarantine policy written down.

**L8.4 — Greppability repair (⏱ 2 h).** Find the worst indirection in your codebase (barrel file, re-export chain, dynamic dispatch by string). Remove one. Measure the difference in agent file-finding on three tasks before and after.
*Success:* before/after with a measured difference — or an honest null result.

**L8.5 — Convention extraction (⏱ 2 h).** Have an agent infer your codebase's implicit conventions from the code itself. Review the list: what it got right tells you what is legible; what it got wrong tells you what is folklore. Encode the important ones as rules or lint checks (M9).
...
<details><summary>Name four things agents want from a codebase and the single property underneath all of them.</summary>

Explicit types over inference; greppability; explicit dependencies; braces over significant whitespace; result types over exceptions; boring conventional structure. Underneath: local reasoning — agents work from a few loaded files with little spatial awareness of the codebase.

</details>

<details><summary>Why is a flaky test suite worse than no test suite?</summary>

It destroys the fitness function. A flaky suite teaches the agent that failures are noise, so it stops treating red as information.

</details>

<details><summary>What does DHH's Rails argument generalize to, beyond Rails?</summary>

The more your codebase looks like the idiom the model already knows, the less context you spend explaining yourself. Convention over configuration is a machine-readable interface.

</details>

<details><summary>Name three things that break greppability.</summary>

Barrel files and re-export chains; import aliasing; string-keyed dynamic dispatch; heavy macros or codegen where the source of truth isn't the source.

</details>

<details><summary>Steinberger spends ~20% of his time on refactoring done entirely by agents. Why is that the most agent-appropriate work there is?</summary>

It's mechanical, verifiable, high-volume and low-judgment — dead code removal, duplicate consolidation, deprecation sweeps.
...
**Hooks are the under-used mechanism.** High-value hooks: format after every edit; run the fast lane after every edit and feed failures back; block edits to secrets, migrations, or vendored directories; log every session for M1 telemetry; refuse test-file modification during implementation-only tasks (M4's anti-gaming guardrail); notify on session end. Each converts vigilance into infrastructure.
...
**L9.1 — Correction mining (⏱ 2 h).** Review your last 20 sessions and list every correction you made. Cluster them. For each cluster, choose the right mechanism and implement the top three.
*Success:* a correction taxonomy plus three implemented extensions, each traced to specific incidents.

**L9.2 — Build a guardrail hook (⏱ 2 h).** Implement a blocking hook for something that must never happen in your repo. Verify it by trying to make the agent do it.
*Success:* a demonstrated block, plus a note on what the agent did when blocked (this is informative).

**L9.3 — Build a review subagent (⏱ 2 h).** Create a specialized reviewer with its own instructions and a specified return format. Run it on five PRs. Tune based on misses.
*Success:* subagent definition, five reports, one tuning iteration documented.

**L9.4 — MCP cost/benefit (⏱ 1.5 h).** Measure the context cost of each connected server. For the most expensive, try replacing it with CLI calls and compare results and tokens.
...
<details><summary>State the difference between an instruction and a hook in one sentence.</summary>

An instruction is a request; a hook is a guarantee. 'Never edit .env' in a memory file is a request the model will usually honour; a pre-tool hook that blocks the edit is enforcement.

</details>

<details><summary>Give the trigger for each: memory file, skill, subagent, hook, plugin.</summary>

Agent gets a convention wrong twice → memory file. Same prompt typed repeatedly, or a playbook pasted a third time → skill. A side task floods your conversation → subagent. Something must happen without asking → hook. A second repo needs the same setup → plugin.

</details>

<details><summary>What makes a subagent worth its overhead, and how do you measure it?</summary>

The compression ratio — tokens read divided by tokens returned. An unspecified return gives you either a transcript (no compression) or a vague summary (lost information).

</details>

<details><summary>When is MCP the right answer and when is a CLI better?</summary>

MCP when the integration needs live connection, authentication, or actions a CLI cannot provide. A CLI when the model already knows the tool from training. Measure the context cost either way.

</details>

<details><summary>What is the configuration cargo cult, and what is the test against it?</summary>

Copying someone else's extension kit, importing their codebase's assumptions and their model's quirks. Every item in your kit should trace to a specific failure you personally experienced.
...
Before Phase 4: a context budget with measurements, a research→plan→implement change landed in unfamiliar code, a legibility audit with executed remediation, and an extension kit whose every item traces to a real failure. **Capstone B** is taken here. See `ASSESSMENT.md` §Gate 2.
...
*Goal of this phase: stop being the bottleneck — safely. Everything here multiplies both output and blast radius, which is why M13 is a hard prerequisite for the unattended portions.*
...
| Cloud/remote sessions | full + off-machine | varies | long jobs, mobile (M12) | weakest observability |
...
**L10.1 — Isolation bake-off (⏱ 3 h).** Run the same three-task batch three ways: same-folder, worktrees, containers. Measure setup time, collisions, wall-clock to all-merged, and your own stress level.
*Success:* a comparison table and a written per-situation policy.

**L10.2 — Find your ceiling (⏱ 3 h).** Run 2, then 3, then 5, then 7 concurrent agents on independent tasks. Track throughput (tasks merged/hour) and quality (rework passes). Find where throughput stops rising.
*Success:* a throughput curve with your stated ceiling and the constraint that sets it.

**L10.3 — Decomposition drill (⏱ 2 h).** Take one large feature. Produce a decomposition into ≥4 parallelizable units with explicit file-level independence. Execute it.
*Success:* the decomposition, the merge outcome, and a note on what collided anyway.

**L10.4 — Build the control plane (⏱ 2 h).** Set up your session ergonomics: naming, colours, switching, completion notification.
...
<details><summary>Cherny calls worktrees the biggest unlock; Steinberger runs agents in the same folder. What is each optimizing?</summary>

Worktrees optimize isolation (no interference, clean review, safe abandonment) at the cost of setup and per-tree rebuilds. Same-folder optimizes latency and shared state at the cost of collision risk — tolerable only with atomic commits and tight supervision.

</details>

<details><summary>What sets most people's ceiling at 3–5 concurrent agents?</summary>

Review capacity — not tooling, not quota. Parallelism moves the bottleneck from generation to your attention.

</details>

<details><summary>What makes a task a good parallel candidate?</summary>

Independence in the file dimension *and* the review dimension. Three features in three subsystems parallelize; three refactors of one module do not.

</details>

<details><summary>Name two cases where parallelism is negative-value.</summary>

Exploratory work where each step informs the next; tasks touching the same files; anything you lack the review budget for; anything where you don't yet know what done looks like.

</details>

<details><summary>Why do session ergonomics — naming, colours, one-key switching — matter disproportionately?</summary>

Without them you spend your attention *finding* the agent that needs you. They're the difference between a workable 3 and a workable 6.
...
**The canonical minimal loop.** The technique Geoffrey Huntley popularized in 2025 as "Ralph" is, at its core, a shell loop feeding a specification file to an agent repeatedly — `while :; do cat PROMPT.md | <agent>; done`. Each iteration gets a fresh context window and works from the durable spec plus whatever state is on disk. The insight is not the bash; it is that **fresh contexts working from a persistent specification beat one long context working from memory**, which is the same insight as M6's intentional compaction, mechanized.
...
**Guardrails for unattended operation** (M13 covers the security dimension; these are the operational ones): a hard budget in tokens and wall-clock; a blast-radius fence (which paths may be touched); no force-push, no production credentials, no destructive migrations; a kill switch you can reach from your phone; an append-only log of every action; and a **rollback plan you have actually tested** — untested rollback is a hope.
...
**L11.1 — Minimal loop (⏱ 2 h).** Build the simplest possible loop in a sandboxed repo: spec file, fresh context each iteration, disk state, hard iteration cap. Run it on a well-specified mechanical task.
*Success:* a loop that completes a real task and stops on its own condition, with a log.

**L11.2 — Stop-condition design (⏱ 2 h).** For three different task types, design and implement stop conditions — success predicate, no-progress detector, budget cap. Test each by making it fire.
*Success:* three loops, each demonstrated stopping for each of the three reasons.

**L11.3 — Induce overbaking (⏱ 1.5 h).** Deliberately run a loop past its useful life with an under-specified task. Document the emergent behaviour.
*Success:* a written account of what it invented, and the guardrail that would have prevented it.

**L11.4 — Verified loop (⏱ 3 h).** Build a fan-out/verify loop: worker agents produce, a separate verifier agent with different instructions checks, failures return to the queue. Run it on a real audit or refactor.
*Success:* a run where the verifier caught ≥1 real defect the worker missed, with the evidence.

**L11.5 — Scheduled triage loop (⏱ 3 h).** Build a loop that runs on a schedule against something real — dependency updates, issue triage, failing-test sweep — with escalation for anything it should not decide. Run it for a week.
...
### Common failure modes
Running loops before M13. No budget cap. No persistent state (the loop re-derives everything each iteration). Model grading its own work. Looping exploratory work. Spec quality unexamined. No rollback test.

### Self-check

*Retrieval practice, not assessment. Answer aloud or in writing from memory before expanding — the effort of recalling is what does the work. Nothing is recorded anywhere.*

<details><summary>Give the precondition hiding inside 'arrange it so it can just go forever'.</summary>

An objective, automatable criterion of success. Where that exists, autonomy is available; where it doesn't, autonomy produces confident garbage at scale.

</details>

<details><summary>Name the five components of a working loop, plus the thing they all depend on.</summary>

Trigger, isolation, knowledge, connection, and verification by a *separate* agent — plus persistent state on disk, because the model forgets everything between runs. The loop's memory is the filesystem.

</details>

<details><summary>What is overbaking, and what does it tell you about budget caps?</summary>

A loop run past its useful life starts inventing work — unrequested features, speculative abstractions, in one recorded case unexpected cryptography. An agent with remaining budget and no remaining work will invent work. Budget caps are a correctness measure, not a cost measure.

</details>

<details><summary>Why do fresh contexts working from a persistent spec beat one long context working from memory?</summary>

Each iteration starts at full attention with a correct, durable statement of the goal, instead of degrading attention over an accumulating transcript. It is intentional compaction, mechanized.

</details>

<details><summary>How many stop conditions does a loop need, and why?</summary>

At least two — a success predicate and a failure or budget condition. With one, you are relying on noticing.

</details>

<details><summary>Karpathy says agents are jagged. Which half of the split does loop engineering work in?</summary>

The verifiable, metric-driven half. They remain weak on nuance, on what you actually intended, and on knowing when to ask a clarifying question. Applying loops to that half is the central error of the era.
...
**Where to run, and why.** Local: fastest feedback, full observability, your credentials, your machine at risk. Cloud/remote session: survives your laptop closing, reachable from anywhere, weaker observability, credential questions get serious. CI-triggered: reproducible, auditable, integrates with existing review — and the highest-risk surface for prompt injection (M13: untrusted issue and PR text has been used to drive agents in CI; see the CVE case study).
...
**Credentials.** Cloud and CI agents need secrets to be useful and are the worst place to put them. Minimum: scoped short-lived tokens, no production credentials, egress restrictions, separate identities per agent so actions are attributable, and an audit log. The published CVE case study in M13 is precisely a credential-exfiltration chain through a CI agent.

### Labs

**L12.1 — Handoff (⏱ 1.5 h).** Run one task through the full pattern: interactive brief and plan → async execution → return for review. Time each phase.
*Success:* a phase breakdown with the fraction of wall-clock requiring you.

**L12.2 — Phone drill (⏱ 1.5 h).** Supervise one real async run entirely from a phone. Note every point where you needed more than the small screen allowed.
*Success:* a friction list and ≥2 changes to how the agent reports or asks.

**L12.3 — Review-path design (⏱ 2 h).** Write a policy classifying agent output into auto-merge / fast review / full review, with rules. Apply it to twenty recent agent changes retrospectively and check for misclassification.
*Success:* a written policy, applied, with the misclassification rate and a revision.

**L12.4 — CI agent, safely (⏱ 2 h).** Set up an agent triggered by a repository event, with least-privilege credentials, egress restrictions, no write access to the repo it evaluates, and a documented threat model.
...
<details><summary>What does async multiply, and what does it not?</summary>

It multiplies production and does nothing for review. A dozen agent PRs waiting on Monday morning is not leverage.

</details>

<details><summary>What can you realistically do from a phone, and what does that imply about agent design?</summary>

Approve or reject a plan, answer a blocking question, kill a run, read a summary. So design agents to ask well — a blocking question with three options beats an open one.

</details>

<details><summary>Name three ways to stop the review queue from becoming a backlog.</summary>

Rate-limit production to review capacity; require self-review by a separate verifier before queueing; batch review into blocks; auto-merge classified low-risk categories on policy; escalate only what policy cannot classify.

</details>

<details><summary>Why is CI the highest-risk place to run an agent?</summary>

It processes untrusted input — issue bodies, PR titles, comments — while holding credentials. That is the published CVE chain.
...
> **This module is a hard prerequisite for the unattended portions of M11 and M12.**
...
| Unattended loop (M11) | container, disposable | strict allowlist | minimal, per-agent identity |
...
**Supply chain.** Two directions, both real. *Inbound:* agents have been documented skipping package verification — installing packages without checking provenance, accepting typosquats, adding dependencies casually. Require lockfiles, pin actions to commit SHAs, and make "new dependency" a mandatory review category (M5). *Outbound:* an agent with write access is a commit path into your repository, and therefore into your users' systems.
...
**L13.1 — Trifecta audit (⏱ 2 h).** Enumerate every agent configuration you run — local, CI, cloud, scheduled. For each, mark the three trifecta conditions. For every all-three case, break one and document how.
*Success:* an audit table with a remediation per finding.

**L13.2 — Attack your own agent (⏱ 2.5 h).** In a sandbox with a fake credential, plant injection payloads in places your agent reads: a README, a code comment, a test fixture, a mock issue body, a tool result. See which the agent obeys.
*Success:* a results table of which vectors worked, plus a written defence for each success.

**L13.3 — Build the sandbox (⏱ 2 h).** Create a containerized environment for unattended runs: no host credentials, egress allowlist, mounted work directory only, disposable.
*Success:* a working sandbox, with a verified test that egress to a non-allowlisted host fails.

**L13.4 — Threat model a workflow (⏱ 2 h).** Take your riskiest agentic workflow. Write a threat model: trust boundaries, untrusted inputs, what an attacker gains, what would detect it. Then fix the top finding.
*Success:* a written threat model and one implemented mitigation.

**L13.5 — Supply-chain guardrail (⏱ 1.5 h).** Implement enforcement — a hook or CI check — that flags any agent-added dependency for human review, and pin your CI actions to SHAs.
...
<details><summary>State the lethal trifecta and the defence.</summary>

Access to private data + exposure to untrusted content + ability to communicate externally. All three together are the vulnerability; any two are safe. Break one.

</details>

<details><summary>Walk the CVE-2025-66032 attack chain.</summary>

Crafted issue on a public repo from a malicious GitHub App → authorization check trusts any App actor → injected instructions read the workflow environment → OIDC token extracted → exchanged for repository write access → malicious commit to the action's own source, poisoning downstream consumers.

</details>

<details><summary>What is remarkable about that chain from a model-capability standpoint?</summary>

Nothing in it required a model failure. Every step was the agent doing exactly what it was told by text it had no way to distinguish from your instructions.

</details>

<details><summary>Isolation should rise with what — and what does it usually rise with instead?</summary>

With autonomy. It usually rises with your comfort level, which grows faster than safety does.

</details>

<details><summary>Where does indirect prompt injection arrive from? Name four sources.</summary>

A GitHub issue; a dependency's README; a web page; a code comment; an error message; a log line; an MCP tool result.

</details>

<details><summary>Why is an MCP server two risks rather than one?</summary>

It is code you run, *and* a channel through which attacker-controllable text enters your context. Treat installing one as installing a dependency with your credentials attached.
...
Before Phase 5: a documented parallelism ceiling, a week-long scheduled loop with logs and an assessment, a review policy in force, and a completed trifecta audit with remediations. See `ASSESSMENT.md` §Gate 3.
...
*Goal of this phase: build the things that let you keep improving after the course ends, and carry other people with you.*
...
The answer is an eval suite of your own. It is unglamorous, it takes a weekend, and it is the single highest-leverage thing in Phase 5 — because it converts an endless stream of vibes-based tool decisions into measurements.
...
**L14.1 — Build the suite (⏱ 4 h).** Assemble 10+ golden tasks from your own history with starting commits and success criteria. Include ≥2 known failures.
*Success:* a runnable suite, executable with one command, producing a score.

**L14.2 — Variance study (⏱ 2 h).** Run five tasks five times each on one model. Compute the spread.
*Success:* a variance table and a stated minimum effect size you will believe.

**L14.3 — A controlled comparison (⏱ 2 h).** Compare two models, or two of your own configurations, holding everything else constant. Report results with the variance caveat applied.
...
**L14.4 — Eval your memory file (⏱ 2 h).** Run the suite with your memory file and without it. Then with the trimmed version from L6.2.
...
<details><summary>Why can't public benchmarks tell you whether a change to your setup helped?</summary>

Different task distribution, different codebase, different harness — and harness choice alone moves scores by double digits on the same base model.

</details>

<details><summary>What makes a golden task useful, and why include ones you know fail?</summary>

A pinned starting commit, the brief as you'd really write it, and an automated check where possible. Known failures are the most informative entries — they're where a new model's improvement first becomes visible.

</details>

<details><summary>Why is a single run per task close to worthless?</summary>

Agent runs are stochastic. Most excited claims about a new model being better are within noise. Run 3–5 times and compare distributions.

</details>

<details><summary>Name three of your own configuration choices worth evaluating, not just models.</summary>

Memory file contents; context minimalism vs pre-loading; plan-first vs direct; subagent return formats; verifier strength; reasoning effort.

</details>

<details><summary>If you use a model as judge, what two precautions apply?</summary>

Use a different model family than the one under test, and spot-check its judgments yourself — LLM-as-judge is quietly biased toward its own family's style.
...
M2 built a toy. This module takes it to the point where you could actually use it — and, more importantly, to the point where you can reason about any harness's design decisions, extend a commercial one meaningfully, and build purpose-specific agents for your own workflows.
...
**Compaction, implemented.** Trigger at a threshold *you* set, not at exhaustion. Preserve the M6 list: goal, approach, completed steps, current failure state, critical paths, rejected approaches with reasons. Write the artifact to disk, not just into the message list, so it survives the process.
...
**Permissions and policy.** Classify tools by side effect; allowlist the provably safe; escalate the rest; log everything. This is M13's architecture expressed in code, and building it once teaches you what commercial permission systems are actually doing.
...
**L15.1 — Production-ish agent (⏱ 5 h).** Extend your M2 agent with: threshold-triggered compaction writing to disk, a subagent tool with a specified return format, a permission layer with an allowlist, and resumable session state.
*Success:* it completes a multi-hour task across a compaction boundary and survives a restart.

**L15.2 — Tool result design (⏱ 2 h).** Instrument your agent to log tokens per tool result. Find the worst offender and redesign it. Re-measure.
*Success:* a before/after token profile with a measured reduction and no loss of capability.

**L15.3 — Build a workflow agent (⏱ 3 h).** Build one small specific agent for a real recurring task of yours. Use it for a week.
*Success:* the agent, plus a week's usage notes and an honest keep/kill decision.

**L15.4 — Orchestration comparison (⏱ 2 h).** Implement the same non-trivial task as a sequential pipeline and as fan-out/verify. Compare cost, wall-clock, and quality.
...
<details><summary>What matters more in a tool result: the call design or the result design? Why?</summary>

The result. It's what enters context. Return the minimum supporting the next decision; make errors actionable; truncate with an explicit marker and a way to get more.

</details>

<details><summary>What separates a demo agent from one you'd let run overnight?</summary>

Persistence and resumption — session state on disk, resume from a state file, and idempotent operations so a resumed run doesn't double-apply.

</details>

<details><summary>Give the build-vs-extend-vs-buy rule.</summary>

Extend when your harness has the seam — right about 80% of the time. Build when you need an agent inside your own product, an unusual loop shape, or full control of context. Buy the general coding harness.

</details>

<details><summary>Why prefer one flexible tool over twelve narrow ones?</summary>

Tool definitions cost context, and choice among near-identical tools degrades selection. Fewer, broader tools with good results beat a large menu.
...
**Reviewing at volume.** Layered defence: automated review first (mechanical issues, conventions, security patterns); human attention reserved for the risk-ordered list from M5; policy-based auto-merge for classified low-risk categories; and a hard rule that **the author reviews their agent's output before it reaches a colleague.** Sending unreviewed agent output to a human reviewer moves your comprehension debt onto someone else's balance sheet, and teams notice.

**Comprehension debt at team scale.** Individual comprehension debt (M5) becomes organizational risk when nobody on the team understands a subsystem. Countermeasures: ownership that means comprehension, not just a CODEOWNERS line; incident-driven repayment (after any incident, someone reads the subsystem properly); architectural decision records so intent survives even when line-level understanding doesn't; and plan/research artifacts in-repo (M7) as the durable record of why.
...
**L16.1 — Bottleneck analysis (⏱ 2 h).** Map your team's flow from idea to production. Identify where a 3× generation increase binds. Quantify current capacity at that point.
*Success:* a flow map with a quantified bottleneck and a proposed intervention.

**L16.2 — Team memory file (⏱ 2 h).** Create or overhaul a shared, version-controlled memory file. Derive its contents from real review comments in your repo's history, not from opinion.
*Success:* a merged file whose every rule traces to ≥2 historical review comments.

**L16.3 — Review policy (⏱ 2 h).** Draft a team policy for agent-generated code: what must be human-reviewed, what can auto-merge, what the author must do before requesting review. Circulate for real feedback.
*Success:* a written policy plus documented colleague feedback and a revision.

**L16.4 — Debt register (⏱ 1.5 h).** Build a comprehension-debt register for your team's codebase: subsystem, owner, comprehension level, last-read date, risk. Schedule repayment for the top item.
...
<details><summary>What are DORA's three findings about AI at organizational scale?</summary>

AI amplifies the existing system (strong orgs get stronger, dysfunctional ones get worse faster); there is a J-curve, an initial dip that is the tuition cost of transformation; and there is an instability tax — individual effectiveness rises while delivery stability falls.

</details>

<details><summary>Where does the bottleneck move when generation triples, and what follows from that?</summary>

Review, then integration, then deployment confidence. Investment belongs at the bottleneck — faster CI, better automated review, trunk hygiene — not in more generation capacity.

</details>

<details><summary>Why must the author review their agent's output before a colleague sees it?</summary>

Otherwise you are moving your comprehension debt onto someone else's balance sheet. Teams notice, and it poisons adoption faster than any technical problem.

</details>

<details><summary>What is the unresolved problem with juniors, stated precisely?</summary>

The traditional apprenticeship ran through exactly the work agents now do. Without deliberately constructed learning paths you produce engineers who can prompt and cannot debug. Nobody has solved this; the course says so rather than inventing an answer.

</details>

<details><summary>Why does promising uniform productivity gains sink a rollout?</summary>

Gains are strongly task-dependent — large on simple work, much smaller on complex legacy code. A uniform promise fails publicly in exactly the codebases that matter most.
...
⊢ M16 (all) · ⏱ 20–30 h (Capstone C runs a week)
...
**Capstone C — Operate an autonomous loop for a week.** Brief in `ASSESSMENT.md`. Real work, real risk, real logs, including everything that went wrong. Prerequisites M11 and M13 are enforced.
...
2. **What I changed my mind about** — compared against `ase/beliefs-2026.md` from M1, quoted
...
<details><summary>Which section of the doctrine document is most valuable, and which do people skip?</summary>

The same one: 'what I am still bad at.' Everything else is easier to write and worth less.
...
<details><summary>Name the four standing practices.</summary>

Monthly: run the eval suite; monthly: one practitioner teardown. Quarterly: re-audit always-on context and the extension kit; quarterly: refresh the state-of-play file.

</details>

<details><summary>What is the actual deliverable of this course?</summary>

Not a skill set — a method for acquiring skill sets faster than they expire. The half-life of technique here is short; the half-life of judgment is long.
...
M1 ──► M2 ──► M3
        ...
        └──► M8  (needs M4)
                   │
     [GATE 2 + Capstone B] ◄── M6,M7,M8,M9
                   │
                   ├──► M10 ──┐
                   │          ├──► M11 (also needs M13) ──► M12
                   ├──► M13 ──┘
                   │
              [GATE 3] ──► M14 ──► M15 ──► M16 ──► M17
```
