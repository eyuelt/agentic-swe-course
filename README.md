...
2. **`CURRICULUM.md`** — 17 modules across 5 stages. Outcomes, teaching content, 70+ labs, mastery artifacts.
3. **Do M0 before anything else.** It is the module people skip and later regret.
...
| `PRACTITIONER-DOSSIER.md` | 10 profiled practitioners + the Teardown protocol + the disagreement map | M2 onward, monthly after |
...
Generation is cheap; verification is the bottleneck. Every durable technique in agentic engineering is a way of closing the gap between *code produced* and *code known-good* faster than the agent widens it. The field has moved through three layers — prompting, then context engineering, now loop engineering — and each subsumes the last. The course teaches all three, plus the permanent obligation that is not a layer: security, comprehension debt, and what happens when a team does this at scale.
...
- **It starts with measurement.** M0 exists because METR found experienced developers were 19% slower with AI while feeling 20% faster — and because the follow-up research is now structurally unable to answer the question for you.
...
This is a course in *doing software engineering with agents*, with enough agent-system design (M1, M14) to make you a better practitioner rather than to make you an agent-framework author. Sources were verified live on 2026-09-17; anything about a specific product is stamped and quarantined.
